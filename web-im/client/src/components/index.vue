<template>
	<div>
		<div class="chat-box">
			<lemon-imui :user="user" ref="IMUI" :width="curWidth" :height="curHeight" :theme="setting.theme"
				:hide-message-name="setting.hideMessageName" :hide-message-time="setting.hideMessageTime"
				:avatarCricle="setting.avatarCricle" @change-contact="handleChangeContact" @send="handleSend"
				:sendKey="setSendKey" :wrapKey="wrapKey"
				@pull-messages="handlePullMessages" style="min-height:600px">
			<!-- <lemon-imui :user="user" ref="IMUI" :width="curWidth" :height="curHeight" :contextmenu="contextmenu"
				...
				<template #sidebar-message="Contact">
					<span class="lemon-badge lemon-contact__avatar" @click="closeGroup">
						<span class="lemon-avatar" v-bind:class="{ 'lemon-avatar--circle': setting.avatarCricle }">
							<img size="50px" :src="Contact.avatar" /></span>
						<span class="lemon-badge__label"
							v-if="Contact.unread > 0 && Contact.is_notice == 1">{{ Contact.unread }}</span>
					</span>
					<div class="lemon-contact__inner" @click="closeGroup">
						...
						<p class="lemon-contact__content lemon-last-content">
							<span class="lastContent">
								<span
									v-if="Contact.is_notice == 0 && Contact.unread > 0">[{{ Contact.unread }}条未读]</span>
								<span v-html="Contact.lastContent"></span>
							</span>
							<span class="el-icon-close-notification f-16" v-if="Contact.is_notice == 0"></span>
						</p>
					</div>
				</template>
						...
						<div>
							<span v-if="isEdit == false">
								<span class="displayName" v-if="is_group == 1">
									<el-tag size="mini">群聊</el-tag> {{ contact.displayName }}<span
										class="mr-5">({{ groupUserCount }})</span>
									<el-tag size="mini" v-if="contact.setting.nospeak == 1"
										type="warning">仅群管理员可发言</el-tag>
							...
							<input v-if="isEdit == true" v-model="displayName" class="editInput" />
						</div>
					</div>
				</template>
				<template #sidebar-message-fixedtop="instance">
					<div class="contact-fixedtop-box">
						<el-input placeholder="搜索" prefix-icon="el-icon-search" @blur="closeSearch"
							@focus="searchResult = true" v-model="keywords" class="input-with-select">
						</el-input>
							...
							<el-dropdown @command="handleCommand">
								...
								<el-dropdown-menu slot="dropdown">
									<el-dropdown-item command="addGroup"
										v-if="globalConfig.chatInfo.groupChat">创建群聊</el-dropdown-item>
								</el-dropdown-menu>
							</el-dropdown>

						</div>
						<div style="margin-left:10px"
							v-if="globalConfig.sysInfo.runMode==1 && globalConfig.chatInfo.groupChat">
							<el-button title="创建群聊" icon="el-icon-plus" @click="openCreateGroup" circle></el-button>
						</div>
						<div class="search-list" v-show="searchResult">
							<div v-for="(item, index) in searchList" :key="index" v-if="searchList.length > 0"
				...
				<template #message-side="contact">
					<div class="slot-group-list" v-if="contact.is_group == 1" v-show="groupResult">
						<div class="group-side-box">
							<div class="group-user">
								<div class="group-side-title">
									<h4>群成员</h4>
									<div>
										<span class="el-icon-circle-plus-outline f-18 cur-handle"
											v-if="contact.role <3 || contact.setting.invite==1"
											@click="openAddGroupUser"></span>
									</div>
								</div>
								<hr />
								<div class="group-user-body" :style="'height:calc('+curHeight+' - 230px )'"
									id="group-user">
									<el-scrollbar style="height:100%;">
										<lemon-contact class="user-list" v-for="(item, index) in groupUser" :key="index"
											:contact="item" v-lemon-contextmenu.contact="groupMenu">
											<div class="user-avatar">
												<el-avatar :size="40" :src="item.userInfo.avatar"></el-avatar>
											</div>
											<div class="user-name">
												<span v-if="item.userInfo.id == user.id"
													class="fc-danger">{{ item.userInfo.displayName }}（我）</span>
												<span v-if="item.userInfo.id != user.id">{{ item.userInfo.displayName}}</span>
											</div>
											<div class="user-role">
												<i class="el-icon-user-solid fc-danger" title="创建者"
													v-if="item.role == 1"></i>
												<i class="el-icon-user-solid fc-warning" title="管理员"
													v-if="item.role == 2"></i>
											</div>
										</lemon-contact>
									</el-scrollbar>
				...
				<template #message-after="message">
					<span v-if="message.fromUser.id == user.id && message.is_group == 2" style="visibility: visible">
		...
		<el-dialog title="发布公告" :visible.sync="noticeBox" :modal="true" width="500px" append-to-body>
			<el-input type="textarea" :rows="10" placeholder="请输入内容" v-model="notice">
			</el-input>
		...
		<el-dialog title="消息管理器" :visible.sync="messageBox" :modal="true" width="800px" append-to-body>
		</el-dialog>
		<el-dialog title="群设置" :visible.sync="groupSetting" :modal="true" width="500px" append-to-body>
...
<script>
	...
	export default {
		...
		methods: {
			...
			wrapKey(e) {
				return this.setting.sendKey == 1 ? (e.keyCode == 13 && e.ctrlKey) : (e.keyCode == 13 && !e
					.ctrlKey && !e.shiftKey);
			},
			// 设置发送键
			setSendKey(e) {
				return this.setting.sendKey == 1 ? (e.keyCode == 13 && !e.ctrlKey && !e.shiftKey) : (e.keyCode ==
					13 && e.ctrlKey);
			},
			//自定义消息的发送
			diySendMessage(message, file) {
				...
				this.handleSend(message, function() {
					var replaceMessage = arguments.length > 0 && arguments[0] !== undefined ? arguments[
						0] : {
							status: "succeed"
						};
					IMUI.updateContact({
